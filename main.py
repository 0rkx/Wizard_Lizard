...
key_statcord = "statcord.com-ojBt9AMyayKIyQ6dhCt3"
api = statcord.Client(client, key_statcord)
api.start_loop()
conn = sqlite3.connect("cogs/dbs/main.db")
...
async def on_message(message):
    if message.author == client.user:
        ...
        if message.content.lower().startswith(f'{get_prefix}'):
...
async def on_ready():
    ...
    await client.change_presence(
        activity=discord.Game(f"w!help in {len(client.guilds)} servers"))
...
def getMeme():
    reddit = praw.Reddit(client_id="haeimp-lUnhNcw",
                         client_secret="nZL64R7MdQg8lc0qa3XFtnsnT7zTpQ",
                         username="0rkx_Bot",
                         password="Obaid2020",
                         user_agent="0rxBot")
...
async def help(ctx):
    embed = discord.Embed(title="Help",
                          url="http://wizardlizard.ml/",
                          description="Help Command",
                          color=0x856319)
    embed.set_author(name=f"{ctx.author}")
    embed.set_thumbnail(
        url=
        "https://images-ext-1.discordapp.net/external/dqyM9Kbbc5kZri2GcBm87GZsNHwP_rJf-7Z4BPlBVd0/%3Fsize%3D1024/https/cdn.discordapp.com/icons/832934847735005224/092ea39443d2ffe13c3a30dedc61c4cb.webp?width=411&height=411"
    )
    embed.add_field(name="Economy",
                    value="All Economy Related Commands",
                    inline=True)
    ...
    embed.set_footer(
        text=f"use help [command] for more information on a command")
    await ctx.send(embed=embed)
...
keep_alive()
client.run("ODMyNjkyODU1NzU5MDQ0NjE4.YHnfpw.ub7xAejTjkWCAhLR52mYRLBXjvs")
